...
var membersModel = require('./routes/model_member.js');
...
app.post('/new_request_friend',friend.new_request_friend); //친구요청
app.post('/search_page_search_key', search.search_key); // 검색리스트
app.post('/request_friend_list',search.request_friend_list); //내가보낸 친구요청
app.post('/response_friend_list',search.response_friend_list); //나에게온 친구 요청
app.post('/new_add_friend',friend.new_add_friend); //친구요청

app.post('/load_timeline',timeline.load_timeline); //타임라인 로드
app.post('/write_timeline',timeline.write_timeline); //글 등록
app.post('/write_comment',timeline.write_comment); //댓글등록
...
app.post('/cover_files',function(req,res){
	console.log(JSON.stringify(req.files));
	var userid= req.param('form_upload_userid');
	...
	require('fs').rename(req.files.cover_change_id.path,
						...
						function(error) {
							if(error) {
								...
								membersModel.findOneAndUpdate({_id: userid},{cover_photo: serverPath},function(err,data)
								{
									if(err){
	...
	require('fs').rename(req.files.profile_change_id.path,
						...
						function(error) {
							if(error) {
								...
								membersModel.findOneAndUpdate({_id: userid},{photo: serverPath},function(err,data)
								{
									if(err){
										//res.json({status:"변경중 오류가 발생했습니다."});
										throw err;
